import"./ProductDialog.astro_astro_type_script_index_0_lang.85w2jD1L.js";import"https://unpkg.com/@supabase/supabase-js@2";import"./AdminLayout.astro_astro_type_script_index_3_lang.Bf11ZY-L.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2";let r;function c(e){try{const t=new Date(e);if(isNaN(t.getTime()))return"Invalid Date";const s=String(t.getMonth()+1).padStart(2,"0"),o=String(t.getDate()).padStart(2,"0"),a=t.getFullYear();return`${s}/${o}/${a}`}catch(t){return console.error("Error formatting date:",t),"Invalid Date"}}function u(){return new Promise(e=>{window.supabase&&window.supabase.auth&&typeof window.supabase.auth.getUser=="function"?(r=window.supabase,e(r)):window.supabase&&typeof window.supabase.createClient=="function"?(r=window.supabase.createClient("https://lmrrdcaavwwletcjcpqv.supabase.co","eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxtcnJkY2Fhdnd3bGV0Y2pjcHF2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU1MDQ0ODgsImV4cCI6MjA3MTA4MDQ4OH0.AU59Qfr6K9i880Gcn5y-3pjCf8PXsDIq4OI0-lPQVuQ"),e(r)):setTimeout(()=>u().then(e),100)})}console.log("Admin data dashboard loaded successfully");document.addEventListener("DOMContentLoaded",async()=>{console.log("🔄 Loading admin data dashboard...");try{console.log("⏳ Waiting for Supabase to load..."),await u(),console.log("✅ Supabase client ready"),await Promise.all([v(),b(),h(),_(),$()]),console.log("✅ All admin data loaded successfully")}catch(e){console.error("❌ Error loading admin data:",e)}});async function v(){try{console.log("🔄 Loading users data...");const{data:e,error:t}=await r.from("profiles").select("*").order("created_at",{ascending:!1});if(t){console.error("Error loading users:",t);return}const s=document.getElementById("users-table-body");if(!s)return;e&&e.length>0?s.innerHTML=e.map(o=>`
          <tr>
        ...
        `).join(""):s.innerHTML='<tr><td colspan="7" class="px-6 py-4 text-center text-gray-500">No users found</td></tr>'}catch(e){console.error("Error loading users data:",e)}}async function b(){try{console.log("🔄 Loading contact form data...");const{data:e,error:t}=await r.from("contact_submissions").select("*").order("created_at",{ascending:!1});if(t){console.error("Error loading contact data:",t);return}const s=document.getElementById("contact-table-body");if(!s)return;e&&e.length>0?s.innerHTML=e.map(o=>`
          <tr>
        ...
        `).join(""):s.innerHTML='<tr><td colspan="7" class="px-6 py-4 text-center text-gray-500">No contact submissions found</td></tr>'}catch(e){console.error("Error loading contact data:",e)}}async function h(){try{if(console.log("🔄 Loading support tickets data..."),r||await u(),!r&&window.supabase&&(window.supabase.auth&&typeof window.supabase.auth.getUser=="function"?r=window.supabase:typeof window.supabase.createClient=="function"&&(r=window.supabase.createClient("https://lmrrdcaavwwletcjcpqv.supabase.co","eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxtcnJkY2Fhdnd3bGV0Y2pjcHF2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU1MDQ0ODgsImV4cCI6MjA3MTA4MDQ4OH0.AU59Qfr6K9i880Gcn5y-3pjCf8PXsDIq4OI0-lPQVuQ"))),!r){console.error("❌ Supabase client not available for loading tickets");const o=document.getElementById("tickets-table-body");o&&(o.innerHTML='<tr><td colspan="7" class="px-6 py-4 text-center text-red-500">Error: Database connection failed</td></tr>');return}const{data:e,error:t}=await r.from("support_tickets").select("*").order("created_at",{ascending:!1});if(t){console.error("Error loading tickets data:",t);return}const s=document.getElementById("tickets-table-body");if(!s)return;e&&e.length>0?s.innerHTML=e.map(o=>`
          <tr>
        ...
        `).join(""):s.innerHTML='<tr><td colspan="7" class="px-6 py-4 text-center text-gray-500">No support tickets found</td></tr>'}catch(e){console.error("Error loading tickets data:",e)}}async function _(){try{if(console.log("🔄 Loading cart customizations data..."),console.log("🔍 Supabase client available:",!!r),!r){console.error("❌ Supabase client not available");const i=document.getElementById("customizations-table-body");i&&(i.innerHTML='<tr><td colspan="7" class="px-6 py-4 text-center text-red-500">Supabase client not available</td></tr>');return}console.log("🔍 Fetching customization_forms data...");const{data:e,error:t}=await r.from("customization_forms").select("*").order("created_at",{ascending:!1});console.log("🔍 Fetching order_customizations data...");const{data:s,error:o}=await r.from("order_customizations").select("*").order("created_at",{ascending:!1});let a=[];if(e&&!t?(console.log("✅ Regular customizations loaded:",e.length),a=[...a,...e]):t&&console.error("❌ Error loading regular customizations:",t),s&&!o){console.log("✅ OMS customizations loaded:",s.length);const i=s.map(n=>({id:n.id,contact_email:n.email,product_type:"order-menu-system",product_name:"Order Menu System",project_name:n.project_name||"Not provided",restaurant_name:n.restaurant_name||"Not provided",owner_name:n.owner_name||"Not provided",restaurant_address:n.address_line_1,contact_person:n.contact_person||"Not provided",phone:n.phone_number||"Not provided",contact_phone:n.phone_number||"Not provided",additional_requirements:n.additional_requirements||"Not provided",primary_color:null,secondary_color:null,accent_color:null,text_color:null,product_price:n.total_amount,created_at:n.created_at,updated_at:n.updated_at,menu_categories:n.menu_categories,menu_items:n.menu_items,logo_url:n.restaurant_logo_url,logo_filename:n.logo_filename,logo_size:n.logo_size}));a=[...a,...i]}else o&&console.error("❌ Error loading OMS customizations:",o);a.sort((i,n)=>new Date(n.created_at)-new Date(i.created_at)),console.log("🔍 Combined customizations data:",a.length,"total items");const l=document.getElementById("customizations-table-body");if(!l){console.error("❌ Customizations table body not found");return}a&&a.length>0?(console.log(`✅ Found ${a.length} customizations`),l.innerHTML=a.map(i=>`
          <tr>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${c(i.created_at)}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
              <button onclick="viewCustomizationDetails('${i.id}')" class="text-blue-600 hover:text-blue-900">View Details</button>
            </td>
          </tr>
        `).join("")):(console.log("ℹ️ No customizations found"),l.innerHTML='<tr><td colspan="7" class="px-6 py-4 text-center text-gray-500">No customizations found</td></tr>')}catch(e){console.error("❌ Error loading customizations data:",e);const t=document.getElementById("customizations-table-body");t&&(t.innerHTML=`<tr><td colspan="7" class="px-6 py-4 text-center text-red-500">Error: ${e.message}</td></tr>`)}}async function $(){try{const[e,t,s,o]=await Promise.all([r.from("profiles").select("*",{count:"exact",head:!0}),r.from("contact_submissions").select("*",{count:"exact",head:!0}),r.from("support_tickets").select("*",{count:"exact",head:!0}),r.from("customization_forms").select("*",{count:"exact",head:!0})]),a=e.count||0,l=t.count||0,i=s.count||0,n=o.count||0,p=document.getElementById("total-users"),m=document.getElementById("total-contacts"),g=document.getElementById("total-tickets"),d=document.getElementById("total-orders");p&&(p.textContent=a.toString()),m&&(m.textContent=l.toString()),g&&(g.textContent=i.toString()),d&&(d.textContent=n.toString()),console.log("✅ Summary cards updated")}catch(e){console.error("Error updating summary cards:",e)}}document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("details-modal");document.getElementById("close-modal")?.addEventListener("click",()=>{e.classList.add("hidden"),document.body.style.overflow=""}),e?.addEventListener("click",s=>{s.target===e&&(e.classList.add("hidden"),document.body.style.overflow="")})});window.viewUserDetails=async function(e){console.log("Viewing user details for:",e);try{const{data:t,error:s}=await r.from("profiles").select("*").eq("id",e).single();if(s){console.error("Error loading user details:",s);return}const o=document.getElementById("details-modal"),a=document.getElementById("modal-title"),l=document.getElementById("modal-content");a.textContent="User Details",l.innerHTML=`
        <div class="space-y-4">
      ...
      `,document.body.style.overflow="hidden",o.classList.remove("hidden")}catch(t){console.error("Error viewing user details:",t)}};window.viewContactDetails=async function(e){console.log("Viewing contact details for:",e);try{const{data:t,error:s}=await r.from("contact_submissions").select("*").eq("id",e).single();if(s){console.error("Error loading contact details:",s);return}const o=document.getElementById("details-modal"),a=document.getElementById("modal-title"),l=document.getElementById("modal-content");a.textContent="Contact Form Details",l.innerHTML=`
        <div class="space-y-4">
          <div class="bg-gray-50 p-4 rounded-lg">
      ...
      `,document.body.style.overflow="hidden",o.classList.remove("hidden")}catch(t){console.error("Error viewing contact details:",t)}};window.viewTicketDetails=async function(e){console.log("Viewing ticket details for:",e);try{const{data:t,error:s}=await r.from("support_tickets").select("*").eq("id",e).single();if(s){console.error("Error loading ticket details:",s);return}const o=document.getElementById("details-modal"),a=document.getElementById("modal-title"),l=document.getElementById("modal-content");a.textContent=`Support Ticket #${t.id}`,l.innerHTML=`
        <div class="space-y-4">
          <div class="bg-gray-50 p-4 rounded-lg">
      ...
      `,document.body.style.overflow="hidden",o.classList.remove("hidden")}catch(t){console.error("Error viewing ticket details:",t)}};window.viewCartDetails=async function(e){console.log("Viewing cart details for:",e);try{const{data:t,error:s}=await r.from("customization_forms").select("*").eq("id",e).single();if(s){console.error("Error loading cart details:",s);return}const o=document.getElementById("details-modal"),a=document.getElementById("modal-title"),l=document.getElementById("modal-content");a.textContent="Cart Customization Details",l.innerHTML=`
        <div class="space-y-4">
          <div class="bg-gray-50 p-4 rounded-lg">
      ...
      `,document.body.style.overflow="hidden",o.classList.remove("hidden")}catch(t){console.error("Error viewing cart details:",t)}};function C(){console.log("🔄 Opening contact forms modal...");const e=document.getElementById("contactFormsModal");e?(e.classList.remove("hidden"),document.body.style.overflow="hidden",console.log("✅ Modal opened, loading contact forms..."),E()):console.error("❌ Contact forms modal not found")}function I(){const e=document.getElementById("contactFormsModal");e&&(e.classList.add("hidden"),document.body.style.overflow="")}async function E(){try{console.log("🔄 Loading contact forms from contact_submissions table...");const{data:e,error:t}=await r.from("contact_submissions").select("*").order("created_at",{ascending:!1});if(t){console.error("❌ Error loading contact forms:",t),alert("Error loading contact forms: "+t.message);return}console.log("✅ Contact forms loaded successfully:",e?.length||0,"contacts found");const s=document.getElementById("contacts-list"),o=document.getElementById("no-contacts");if(!e||e.length===0){s.classList.add("hidden"),o.classList.remove("hidden");return}o.classList.add("hidden"),s.classList.remove("hidden"),s.innerHTML=e.map(a=>`
      ...
      `).join("")}catch(e){alert("Error loading contact forms: "+e.message)}}function N(){const e=document.getElementById("contactSearch").value.toLowerCase();document.querySelectorAll("#contacts-list > div").forEach(s=>{const o=s.querySelector('p[class*="text-gray-500"]')?.textContent.toLowerCase()||"",a=s.querySelector("h4")?.textContent.toLowerCase()||"",l=s.querySelector('p[class*="text-gray-600"]')?.textContent.toLowerCase()||"";o.includes(e)||a.includes(e)||l.includes(e)?s.style.display="block":s.style.display="none"})}async function L(e){try{const{data:t,error:s}=await r.from("contact_submissions").select("*").eq("id",e).single();if(s){alert("Error loading contact details: "+s.message);return}document.getElementById("contacts-list").classList.add("hidden"),document.getElementById("contact-details-view").classList.remove("hidden"),setTimeout(()=>{const a=document.getElementById("contact-details-view");if(a){a.replaceWith(a.cloneNode(!0));const l=document.getElementById("contact-details-view");l.addEventListener("click",function(i){i.target===l&&y()})}},100);const o=document.getElementById("contact-details-content");o.innerHTML=`
      ...
      `}catch(t){console.error("Error viewing contact details:",t),alert("Error loading contact details: "+t.message)}}function y(){document.getElementById("contact-details-view").classList.add("hidden"),document.getElementById("contacts-list").classList.remove("hidden")}function k(){console.log("🔄 Opening customization forms modal...");const e=document.getElementById("customizationFormsModal");e?(e.classList.remove("hidden"),document.body.style.overflow="hidden",console.log("✅ Modal opened, loading customization forms..."),z()):console.error("❌ Customization forms modal not found")}function j(){const e=document.getElementById("customizationFormsModal");e&&(e.classList.add("hidden"),document.body.style.overflow="")}async function z(){try{console.log("🔄 Loading customization forms from order_customizations table...");const{data:e,error:t}=await r.from("order_customizations").select("*").order("created_at",{ascending:!1});if(t){console.error("❌ Error loading customization forms:",t),alert("Error loading customization forms: "+t.message);return}console.log("✅ Customization forms loaded successfully:",e?.length||0,"forms found");const s=document.getElementById("customizations-list"),o=document.getElementById("no-customizations");if(!e||e.length===0){s.classList.add("hidden"),o.classList.remove("hidden");return}o.classList.add("hidden"),s.classList.remove("hidden"),s.innerHTML=e.map(a=>`
        <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
          <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
            ...
            <span class="text-sm text-gray-500 break-words overflow-wrap-anywhere">${c(a.created_at)}</span>
          </div>
          
          <h4 class="font-semibold text-gray-900 mb-2 break-words overflow-wrap-anywhere">${a.project_name||"No Project Name"}</h4>
          <div class="text-gray-600 text-sm mb-2">
            <p class="mb-1 break-words overflow-wrap-anywhere"><strong>Restaurant Name:</strong> <span class="break-words overflow-wrap-anywhere">${a.restaurant_name||"Not provided"}</span></p>
            <p class="mb-1 break-words overflow-wrap-anywhere"><strong>Owner Name:</strong> <span class="break-words overflow-wrap-anywhere">${a.owner_name||"Not provided"}</span></p>
            <p class="mb-1 break-words overflow-wrap-anywhere"><strong>Contact Person:</strong> <span class="break-words overflow-wrap-anywhere">${a.contact_person||"Not provided"}</span></p>
            <p class="mb-1 break-words overflow-wrap-anywhere"><strong>Total Amount:</strong> ₹${(a.total_amount||0).toLocaleString("en-IN")}</p>
            <p class="mb-1 break-words overflow-wrap-anywhere"><strong>Additional Requirements:</strong> <span class="break-words overflow-wrap-anywhere">${a.additional_requirements?a.additional_requirements.length>100?a.additional_requirements.substring(0,100)+"...":a.additional_requirements:"No additional requirements provided"}</span></p>
          </div>
          <p class="text-gray-500 text-xs mb-3 break-words overflow-wrap-anywhere">Email: ${a.email||"Not provided"}</p>
          <p class="text-gray-400 text-xs mb-3 break-words overflow-wrap-anywhere">Phone: ${a.phone_number||"Not provided"}</p>
      ...
      `).join("")}catch(e){alert("Error loading customization forms: "+e.message)}}function M(){const e=document.getElementById("customizationSearch").value.toLowerCase();document.querySelectorAll("#customizations-list > div").forEach(s=>{const o=s.querySelector('p[class*="text-gray-500"]')?.textContent.toLowerCase()||"",a=s.querySelector("h4")?.textContent.toLowerCase()||"",l=s.querySelector('p[class*="text-gray-600"]')?.textContent.toLowerCase()||"";o.includes(e)||a.includes(e)||l.includes(e)?s.style.display="block":s.style.display="none"})}async function S(e){try{console.log("🔄 Loading customization details for:",e);const{data:t,error:s}=await r.from("order_customizations").select("*").eq("id",e).single();if(s){alert("Error loading customization details: "+s.message);return}document.getElementById("customizations-list").classList.add("hidden"),document.getElementById("customization-details-view").classList.remove("hidden"),setTimeout(()=>{const d=document.getElementById("customization-details-view");if(d){d.replaceWith(d.cloneNode(!0));const x=document.getElementById("customization-details-view");x.addEventListener("click",function(w){w.target===x&&f()})}},100);const o=document.getElementById("customization-details-content");let a=t.menu_photos_urls||[];try{typeof a=="string"&&(a=JSON.parse(a))}catch{}const l=t.restaurant_logo_url||t.logo_url||"";let i=!1;(!Array.isArray(a)||a.length===0)&&l&&(a=[{url:l,filename:"restaurant-logo"}],i=!0);const n=Array.isArray(a)&&a.length?`<div class="grid grid-cols-3 sm:grid-cols-4 gap-3">${a.map(d=>`
          ...
          `).join("")}</div>`:'<p class="text-gray-500">No menu photos uploaded</p>',p=Array.isArray(a)&&a.length>0,m=(!!t.restaurant_logo_url||!!t.logo_url)&&!p&&!i,g=m?`<div data-image-url="${t.restaurant_logo_url||t.logo_url}" class="image-view-trigger cursor-pointer hover:opacity-80 transition-opacity inline-block">
