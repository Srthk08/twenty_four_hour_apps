import{supabase as c}from"./supabase.B9-RI-CL.js";import"./ProductDialog.astro_astro_type_script_index_0_lang.C1LHbjNP.js";import"https://unpkg.com/@supabase/supabase-js@2";import"./AdminLayout.astro_astro_type_script_index_3_lang.LJYybVYx.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2";import"./index.BymsOcZI.js";import"./preload-helper.CLcXU_4U.js";class f{constructor(){}static getInstance(){return f.instance||(f.instance=new f),f.instance}async getUsers(){try{const{data:e,error:t}=await c.from("profiles_complete").select("*").order("created_at",{ascending:!1});return t?(console.error("Error fetching users:",t),[]):e||[]}catch(e){return console.error("Error in getUsers:",e),[]}}async getUserById(e){try{const{data:t,error:r}=await c.from("profiles_complete").select("*").eq("id",e).single();return r?(console.error("Error fetching user by ID:",r),null):t}catch(t){return console.error("Error in getUserById:",t),null}}async addUser(e){try{const{data:t,error:r}=await c.from("user_profiles").insert({email:e.email,full_name:e.full_name,phone:e.phone,company_name:e.company_name,role:e.role,status:e.status,username:e.username,avatar_url:e.avatar_url,bio:e.bio,website:e.website,location:e.location,timezone:e.timezone,language:e.language}).select().single();return r?(console.error("Error adding user:",r),null):t}catch(t){return console.error("Error in addUser:",t),null}}async updateUser(e,t){try{const{data:r,error:n}=await c.from("user_profiles").update({...t,updated_at:new Date().toISOString()}).eq("id",e).select().single();return n?(console.error("Error updating user:",n),null):r}catch(r){return console.error("Error in updateUser:",r),null}}async deleteUser(e){try{const{error:t}=await c.from("profiles").delete().eq("id",e);return t?(console.error("Error deleting user:",t),!1):!0}catch(t){return console.error("Error in deleteUser:",t),!1}}async getOrders(){try{const{data:e,error:t}=await c.from("orders").select(`
    ...
    `).join(""),x()}function B(){const a=document.getElementById("transactions-count");a&&(a.textContent=`Showing ${g.length} of ${y.length} transactions`)}function x(){const a=document.getElementById("page-numbers"),e=document.getElementById("prev-page"),t=document.getElementById("next-page"),r=document.getElementById("page-numbers-mobile"),n=document.getElementById("prev-page-mobile"),s=document.getElementById("next-page-mobile");a&&e&&t&&I(a,e,t),r&&n&&s&&I(r,n,s)}function I(a,e,t){e.disabled=l===1,e.classList.toggle("opacity-50",l===1),e.classList.toggle("cursor-not-allowed",l===1),t.disabled=l===d||d===0,t.classList.toggle("opacity-50",l===d||d===0),t.classList.toggle("cursor-not-allowed",l===d||d===0);let r="";const n=5;let s=Math.max(1,l-Math.floor(n/2)),o=Math.min(d,s+n-1);o-s+1<n&&(s=Math.max(1,o-n+1));for(let u=s;u<=o;u++)r+=`
        <button 
...
`),r=new Blob([t],{type:"text/csv"}),n=window.URL.createObjectURL(r),s=document.createElement("a");s.href=n,s.download=`transactions-${new Date().toISOString().split("T")[0]}.csv`,s.click(),window.URL.revokeObjectURL(n)}catch(a){alert("Failed to export transactions"),console.error("Error exporting transactions:",a)}}function M(){document.getElementById("search")?.addEventListener("input",h),document.getElementById("status-filter")?.addEventListener("change",h),document.getElementById("date-filter")?.addEventListener("change",h),document.getElementById("prev-page")?.addEventListener("click",E),document.getElementById("next-page")?.addEventListener("click",v),document.getElementById("refresh-transactions")?.addEventListener("click",w),document.getElementById("prev-page-mobile")?.addEventListener("click",E),document.getElementById("next-page-mobile")?.addEventListener("click",v),document.getElementById("refresh-transactions-mobile")?.addEventListener("click",w),document.getElementById("export-btn")?.addEventListener("click",P),document.getElementById("close-modal")?.addEventListener("click",()=>{document.getElementById("transaction-modal")?.classList.add("hidden")}),document.getElementById("transaction-modal")?.addEventListener("click",a=>{a.target===a.currentTarget&&a.currentTarget.classList.add("hidden")})}function A(){const a=document.getElementById("items-per-page"),e=document.getElementById("items-per-page-mobile");a&&e&&(e.addEventListener("change",t=>{t.preventDefault(),a.value=e.value,p=parseInt(e.value),l=1,m()}),a.addEventListener("change",t=>{t.preventDefault(),e.value=a.value,p=parseInt(a.value),l=1,m()}))}document.addEventListener("DOMContentLoaded",()=>{T(),A()});window.viewTransaction=O;window.refundTransaction=R;window.goToPage=L;window.nextPage=v;window.prevPage=E;window.changeItemsPerPage=$;
