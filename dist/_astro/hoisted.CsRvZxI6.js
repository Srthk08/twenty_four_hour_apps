import"./ProductDialog.astro_astro_type_script_index_0_lang.85w2jD1L.js";import"https://unpkg.com/@supabase/supabase-js@2";import"./AdminLayout.astro_astro_type_script_index_3_lang.Bf11ZY-L.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2";let y;function L(){return new Promise(t=>{window.supabase&&window.supabase.createClient?(y=window.supabase.createClient("https://lmrrdcaavwwletcjcpqv.supabase.co","eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxtcnJkY2Fhdnd3bGV0Y2pjcHF2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU1MDQ0ODgsImV4cCI6MjA3MTA4MDQ4OH0.AU59Qfr6K9i880Gcn5y-3pjCf8PXsDIq4OI0-lPQVuQ"),t(y)):setTimeout(()=>L().then(t),100)})}let i=[],u=[],r=1,g=10,d=1;async function S(){try{console.log("⏳ Waiting for Supabase to load..."),await L(),console.log("✅ Supabase client ready"),f(),j(),setTimeout(async()=>{console.log("⏰ Delayed real data load in Admin panel..."),await f()},2e3)}catch(t){console.error("❌ Error initializing orders page:",t)}}async function f(){try{console.log("🔄 Loading OMS customization orders from Admin panel..."),console.log("🔍 Supabase client:",y),console.log("🔍 Checking existing data in order_customizations table...");const{data:t,error:e}=await y.from("order_customizations").select("*").limit(5);e?(console.error("❌ Error fetching existing data:",e),console.error("❌ Error details:",e.message),console.error("❌ Error code:",e.code),console.error("❌ Error hint:",e.hint)):(console.log("📊 Existing data in order_customizations:",t?.length||0,"records"),t&&t.length>0&&console.log("📋 Sample data:",t[0]));const{data:a,error:n}=await y.from("order_customizations").select("*").order("created_at",{ascending:!1}).limit(100);console.log("📊 Query result:",{orders:a,error:n}),n?(console.error("❌ Error loading orders:",n),console.error("❌ Error details:",n.message),console.error("❌ Error code:",n.code),console.error("❌ Error hint:",n.hint),i=[{id:"example-order-1",orderNumber:"ORD-2024-001",customerName:"John Doe",customerEmail:"john.doe@example.com",serviceName:"Web Development",serviceType:"custom",status:"pending",amount:15e3,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}],console.log("⚠️ Using fallback example data")):(console.log("✅ Successfully fetched orders from Supabase:",a?.length||0),!a||a.length===0?(console.log("⚠️ No real data found, using example data"),i=[{id:"example-order-1",orderNumber:"ORD-2024-001",customerName:"John Doe",customerEmail:"john.doe@example.com",serviceName:"Web Development",serviceType:"custom",status:"pending",amount:15e3,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}]):(console.log("✅ Transforming OMS data to order format..."),i=(a||[]).map(s=>({id:s.id,orderNumber:`OMS-${s.id.slice(-8).toUpperCase()}`,customerName:s.owner_name||"No Name",customerEmail:s.email||"No Email",serviceName:s.project_name||"Order Menu System",serviceType:"oms_customization",status:s.status||"pending",paymentStatus:s.payment_status||"pending",amount:s.total_amount||0,createdAt:s.created_at,updatedAt:s.updated_at||s.created_at,restaurantName:s.restaurant_name,contactPerson:s.contact_person,phoneNumber:s.phone_number,address:s.address_line_1,city:s.city,state:s.state,pincode:s.pincode,country:s.country,additionalRequirements:s.additional_requirements,logoUrl:s.restaurant_logo_url,menuPhotos:s.menu_photos_urls||[]})),console.log("✅ Transformed orders:",i.length),console.log("📋 Sample transformed order:",i[0]))),u=[...i],r=1,m(),w(),console.log(`✅ Final result: ${i.length} orders loaded`)}catch(t){console.error("❌ Exception loading orders:",t)}}function m(){const t=document.getElementById("orders-tbody");if(!t)return;if(u.length===0){t.innerHTML=`
      ...
      `,I();return}d=Math.ceil(u.length/g);const e=(r-1)*g,a=e+g,n=u.slice(e,a);t.innerHTML=n.map(s=>`
    ...
    `).join(""),I()}function w(){const t=document.getElementById("orders-count");t&&(t.textContent=`Showing ${u.length} of ${i.length} orders`)}function I(){const t=document.getElementById("page-numbers"),e=document.getElementById("prev-page"),a=document.getElementById("next-page"),n=document.getElementById("page-numbers-mobile"),s=document.getElementById("prev-page-mobile"),o=document.getElementById("next-page-mobile");t&&e&&a&&N(t,e,a),n&&s&&o&&N(n,s,o)}function N(t,e,a){e.disabled=r===1,e.classList.toggle("opacity-50",r===1),e.classList.toggle("cursor-not-allowed",r===1),a.disabled=r===d||d===0,a.classList.toggle("opacity-50",r===d||d===0),a.classList.toggle("cursor-not-allowed",r===d||d===0);let n="";const s=5;let o=Math.max(1,r-Math.floor(s/2)),c=Math.min(d,o+s-1);c-o+1<s&&(o=Math.max(1,c-s+1));for(let l=o;l<=c;l++)n+=`
        <button 
      ...
      `;t.innerHTML=n}function M(t){t<1||t>d||(r=t,m())}function v(){r<d&&(r++,m())}function h(){r>1&&(r--,m())}function D(){const t=document.getElementById("items-per-page"),e=document.getElementById("items-per-page-mobile");let a=g;t&&(a=parseInt(t.value)),e&&(a=parseInt(e.value)),t&&(t.value=a),e&&(e.value=a),g=a,r=1,m()}function $(t){return{pending:"bg-yellow-100 text-yellow-800",confirmed:"bg-blue-100 text-blue-800",in_progress:"bg-purple-100 text-purple-800",completed:"bg-green-100 text-green-800",cancelled:"bg-red-100 text-red-800"}[t]||"bg-gray-100 text-gray-800"}function x(){const t=document.getElementById("search").value.toLowerCase(),e=document.getElementById("status-filter").value,a=document.getElementById("date-filter").value;u=i.filter(n=>{const s=n.orderNumber||n.id||"",o=n.customerName||n.customer_name||"",c=n.customerEmail||n.customer_email||"",l=n.serviceName||n.service_name||"",p=s.toLowerCase().includes(t)||o.toLowerCase().includes(t)||c.toLowerCase().includes(t)||l.toLowerCase().includes(t),b=!e||n.status===e,E=n.createdAt||n.created_at,O=!a||!E||B(E,a);return p&&b&&O}),m(),w()}function B(t,e){const a=new Date(t),n=new Date;switch(e){case"today":return a.toDateString()===n.toDateString();case"week":const s=new Date(n.getTime()-7*24*60*60*1e3);return a>=s;case"month":const o=new Date(n.getTime()-30*24*60*60*1e3);return a>=o;case"quarter":const c=new Date(n.getTime()-90*24*60*60*1e3);return a>=c;default:return!0}}function P(t){const e=i.find(o=>o.id===t);if(!e){console.error("Order not found:",t),alert("Order not found");return}const a=document.getElementById("order-modal"),n=document.getElementById("modal-title"),s=document.getElementById("modal-content");a&&n&&s&&(n.textContent=`Order ${e.orderNumber}`,s.innerHTML=`
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ${o.map(c=>{const l=typeof c=="string"?c:c.url||c.publicUrl||"",p=typeof c=="object"&&c.filename||"menu-photo";return`
              <div class="relative image-view-trigger cursor-pointer" data-image-url="${l.replace(/"/g,"&quot;").replace(/'/g,"\\'")}">
                <img src="${l}" alt="${p}" class="w-full h-24 object-cover rounded-lg" title="${p}">
        ...
        `:""})()}
      `,document.body.style.overflow="hidden",a.classList.remove("hidden"))}async function _(t){const e=i.find(n=>n.id===t);if(!e){console.error("Order not found:",t),alert("Order not found");return}const a=prompt(`Current status: ${e.status||"No status"}
...
- in_progress
- completed
- cancelled`,e.status||"pending");if(a&&a!==e.status)try{const n=i.findIndex(s=>s.id===t);n>-1&&(i[n].status=a,i[n].updatedAt=new Date().toISOString(),u=[...i],m(),w()),alert("Order status updated successfully!")}catch(n){alert("Failed to update order status"),console.error("Error updating order status:",n)}}function j(){document.getElementById("search")?.addEventListener("input",x),document.getElementById("status-filter")?.addEventListener("change",x),document.getElementById("date-filter")?.addEventListener("change",x),document.getElementById("prev-page")?.addEventListener("click",h),document.getElementById("next-page")?.addEventListener("click",v),document.getElementById("refresh-orders")?.addEventListener("click",async()=>{console.log("🔄 Manual refresh of orders triggered"),await f()}),document.getElementById("prev-page-mobile")?.addEventListener("click",h),document.getElementById("next-page-mobile")?.addEventListener("click",v),document.getElementById("refresh-orders-mobile")?.addEventListener("click",async()=>{console.log("🔄 Manual refresh of orders triggered (mobile)"),await f()}),document.getElementById("close-modal")?.addEventListener("click",()=>{document.getElementById("order-modal")?.classList.add("hidden"),document.body.style.overflow=""}),document.getElementById("order-modal")?.addEventListener("click",t=>{t.target===t.currentTarget&&(t.currentTarget.classList.add("hidden"),document.body.style.overflow="")})}function k(){const t=document.getElementById("items-per-page"),e=document.getElementById("items-per-page-mobile");t&&e&&(e.addEventListener("change",a=>{a.preventDefault(),t.value=e.value,g=parseInt(e.value),r=1,m()}),t.addEventListener("change",a=>{a.preventDefault(),e.value=t.value,g=parseInt(t.value),r=1,m()}))}document.addEventListener("DOMContentLoaded",()=>{S(),k()});window.openImageModal=function(t){console.log("🖼️ Opening image modal for:",t);const e=document.getElementById("image-modal"),a=document.getElementById("image-modal-img");e&&a&&t?(a.src=t,a.onerror=function(){console.error("❌ Failed to load image:",t),closeImageModal()},document.body.style.overflow="hidden",e.classList.remove("hidden")):console.error("❌ Image modal elements not found or invalid URL")};window.closeImageModal=function(){const t=document.getElementById("image-modal");if(t){t.classList.add("hidden"),document.body.style.overflow="";const e=document.getElementById("image-modal-img");e&&(e.src="")}};document.addEventListener("keydown",function(t){t.key==="Escape"&&closeImageModal()});document.addEventListener("DOMContentLoaded",function(){const t=document.getElementById("image-modal");t&&t.addEventListener("click",function(e){e.target===t&&closeImageModal()}),document.addEventListener("click",function(e){const a=e.target.closest(".image-view-trigger");if(a){e.preventDefault(),e.stopPropagation();const n=a.getAttribute("data-image-url");n?openImageModal(n):console.error("❌ No image URL found")}})});window.viewOrder=P;window.updateOrderStatus=_;window.goToPage=M;window.nextPage=v;window.prevPage=h;window.changeItemsPerPage=D;
