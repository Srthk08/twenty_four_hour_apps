import{s as n}from"./supabase.ASU9rNY7.js";import"./Footer.astro_astro_type_script_index_0_lang.Z9Ovqgr-.js";import"https://unpkg.com/@supabase/supabase-js@2";import"./AdminLayout.astro_astro_type_script_index_3_lang.LJYybVYx.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2";import"./index.BFAZBQoJ.js";console.log("Admin data dashboard loaded successfully");document.addEventListener("DOMContentLoaded",async()=>{console.log("🔄 Loading admin data dashboard...");try{await Promise.all([x(),y(),f(),b(),v()]),console.log("✅ All admin data loaded successfully")}catch(e){console.error("❌ Error loading admin data:",e)}});async function x(){try{console.log("🔄 Loading users data...");const{data:e,error:t}=await n.from("profiles").select("*").order("created_at",{ascending:!1});if(t){console.error("Error loading users:",t);return}const o=document.getElementById("users-table-body");if(!o)return;e&&e.length>0?o.innerHTML=e.map(s=>`
          <tr>
