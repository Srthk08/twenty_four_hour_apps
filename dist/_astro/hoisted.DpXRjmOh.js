import"./ProductDialog.astro_astro_type_script_index_0_lang.85w2jD1L.js";import"https://unpkg.com/@supabase/supabase-js@2";import"./AdminLayout.astro_astro_type_script_index_3_lang.Bf11ZY-L.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2";const C="https://lmrrdcaavwwletcjcpqv.supabase.co",N="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxtcnJkY2Fhdnd3bGV0Y2pjcHF2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU1MDQ0ODgsImV4cCI6MjA3MTA4MDQ4OH0.AU59Qfr6K9i880Gcn5y-3pjCf8PXsDIq4OI0-lPQVuQ";function u(s){try{const t=new Date(s);if(isNaN(t.getTime()))return"Invalid Date";const a=String(t.getMonth()+1).padStart(2,"0"),e=String(t.getDate()).padStart(2,"0"),o=t.getFullYear();return`${a}/${e}/${o}`}catch(t){return console.error("Error formatting date:",t),"Invalid Date"}}let n=null;function h(){return new Promise(s=>{window.supabase&&window.supabase.auth&&typeof window.supabase.auth.getUser=="function"?(n=window.supabase,s(n)):window.supabase&&typeof window.supabase.createClient=="function"?(n=window.supabase.createClient(C,N),s(n)):setTimeout(()=>h().then(s),100)})}console.log("Admin dashboard loaded successfully");window.addEventListener("load",()=>{console.log("✅ Admin panel loaded successfully");const s=document.getElementById("customizations-table-body");s&&(s.innerHTML='<tr><td colspan="7" class="px-6 py-4 text-center text-gray-500">Loading customizations...</td></tr>')});document.addEventListener("DOMContentLoaded",async()=>{console.log("🔄 Loading admin dashboard data..."),console.log("✅ Admin dashboard JavaScript is running!"),n||(console.log("⏳ Waiting for Supabase client..."),await h()),console.log("🔍 Supabase client:",n),console.log("🔍 Customizations table body element:",document.getElementById("customizations-table-body"));const s=document.getElementById("users-table-body"),t=document.getElementById("contact-table-body");s&&(s.innerHTML='<tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">Loading users...</td></tr>'),t&&(t.innerHTML='<tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">Loading contacts...</td></tr>');try{console.log("🔍 Testing Supabase connection...");const{data:a,error:e}=await n.from("profiles").select("count",{count:"exact",head:!0});console.log("Profiles table test:",{data:a,error:e});const{data:o,error:d}=await n.from("contact_submissions").select("count",{count:"exact",head:!0});if(console.log("Contact submissions table test:",{data:o,error:d}),e&&(console.error("❌ Profiles table error:",e),alert("Profiles table error: "+e.message)),d){console.error("❌ Contact submissions table error:",d);const i=document.getElementById("contact-table-body");i&&(d.message.includes("Could not find the table")?i.innerHTML=`
              <tr>
        ...
        `).join(""):a.innerHTML='<tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">No users found</td></tr>'}catch(s){console.error("Error loading users data:",s)}}async function j(){try{console.log("🔄 Loading contact form data from contact_submissions table...");const{data:s,error:t}=await n.from("contact_submissions").select("*").order("created_at",{ascending:!1});if(t){console.error("❌ Error loading contact data:",t),console.error("Error details:",{message:t.message,details:t.details,hint:t.hint,code:t.code});const e=document.getElementById("contact-table-body");e&&(e.innerHTML=`
            <tr>
              <td colspan="6" class="px-6 py-4 text-center text-red-500">
        ...
        `).join(""):a.innerHTML='<tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">No contact submissions found</td></tr>'}catch(s){console.error("Error loading contact data:",s)}}async function S(){try{if(console.log("🔄 Loading support tickets data..."),n||await h(),!n&&window.supabase&&(window.supabase.auth&&typeof window.supabase.auth.getUser=="function"?n=window.supabase:typeof window.supabase.createClient=="function"&&(n=window.supabase.createClient(C,N))),!n){console.error("❌ Supabase client not available for loading tickets");const e=document.getElementById("tickets-table-body");e&&(e.innerHTML='<tr><td colspan="6" class="px-6 py-4 text-center text-red-500">Error: Database connection failed</td></tr>');return}const{data:s,error:t}=await n.from("support_tickets").select("*").order("created_at",{ascending:!1});if(t){console.error("Error loading tickets data:",t);const e=document.getElementById("tickets-table-body");e&&(e.innerHTML=`<tr><td colspan="6" class="px-6 py-4 text-center text-red-500">Error: ${t.message}</td></tr>`);return}const a=document.getElementById("tickets-table-body");if(!a)return;s&&s.length>0?a.innerHTML=s.map(e=>`
          <tr>
      ...
      `,document.body.style.overflow="hidden",e.classList.remove("hidden")}catch(t){console.error("Error viewing contact details:",t)}};let p=[],f=[];window.showContactFormDetails=async function(){console.log("Loading all contact form details...");try{const{data:s,error:t}=await n.from("contact_submissions").select("*").order("created_at",{ascending:!1});if(t){console.error("Error loading contact details:",t);return}p=s||[];const a=document.getElementById("contact-details-modal"),e=document.getElementById("contact-modal-title"),o=document.getElementById("contact-modal-content");e.textContent=`All Contact Form Details (${p.length} submissions)`,o.innerHTML=`
        <div class="mb-6">
          <div class="relative">
      ...
      `}window.filterContacts=function(){const s=document.getElementById("contact-search-input"),t=document.getElementById("contact-modal-title"),a=s.value.toLowerCase().trim();if(!a){document.getElementById("contacts-list").innerHTML=b(p),t.textContent=`All Contact Form Details (${p.length} submissions)`;return}const e=p.filter(o=>(o.email||"").toLowerCase().includes(a));document.getElementById("contacts-list").innerHTML=b(e),t.textContent=`Contact Form Details (${e.length} of ${p.length} submissions)`};window.showCustomizations=async function(){console.log("Loading all customizations...");try{const{data:s,error:t}=await n.from("customization_forms").select("*").order("created_at",{ascending:!1}),{data:a,error:e}=await n.from("order_customizations").select("*").order("created_at",{ascending:!1});let o=[];if(s&&!t?o=[...o,...s]:t&&console.error("Error loading regular customizations:",t),a&&!e){const c=a.map(l=>({id:l.id,contact_email:l.email||"Not provided",product_type:"order-menu-system",product_name:"Order Menu System",project_name:l.project_name||"Not provided",restaurant_name:l.restaurant_name||"Not provided",owner_name:l.owner_name||"Not provided",restaurant_address:l.address_line_1,contact_person:l.contact_person||"Not provided",phone:l.phone_number||"Not provided",additional_requirements:l.additional_requirements||"Not provided",primary_color:null,secondary_color:null,accent_color:null,text_color:null,product_price:l.total_amount,created_at:l.created_at,updated_at:l.updated_at,menu_categories:typeof l.menu_categories=="string"?JSON.parse(l.menu_categories):l.menu_categories||[],menu_items:typeof l.menu_items=="string"?JSON.parse(l.menu_items):l.menu_items||[],logo_url:l.restaurant_logo_url,logo_filename:l.logo_filename,logo_size:l.logo_size}));o=[...o,...c]}else e&&console.error("Error loading OMS customizations:",e);o.sort((c,l)=>new Date(l.created_at)-new Date(c.created_at)),window.allCustomizations=o;const d=document.getElementById("customizations-modal"),i=document.getElementById("customizations-modal-title"),r=document.getElementById("customizations-modal-content");i.textContent=`All Product Customizations (${o.length} items)`,r.innerHTML=`
        <div class="mb-6">
          <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      `,document.body.style.overflow="hidden",d.classList.remove("hidden")}catch(s){console.error("Error loading customizations:",s),alert("Error loading customizations: "+s.message)}};function _(s){return s&&s.length>0?s.map((t,a)=>`
      ...
      `}window.filterCustomizations=function(){const s=document.getElementById("customizations-search-input"),t=document.getElementById("customizations-modal-title"),a=s.value.toLowerCase().trim();if(!a){document.getElementById("customizations-list").innerHTML=_(f),t.textContent=`All Product Customizations (${f.length} items)`;return}const e=f.filter(o=>(o.contact_email||"").toLowerCase().includes(a));document.getElementById("customizations-list").innerHTML=_(e),t.textContent=`Product Customizations (${e.length} of ${f.length} items)`};window.loadCustomizationsData=w;window.refreshOMSCustomizations=async function(){console.log("🔄 Refreshing OMS customizations and clearing cache..."),window.allCustomizations=[],await w(),console.log("✅ OMS customizations refreshed")};window.viewCustomizationDetails=async function(s){console.log("🔍 Viewing customization details for ID:",s),console.log("🔍 Supabase client available:",!!n),console.log("🔄 Force refreshing data from order_customizations table...");try{if(!n){console.error("❌ Supabase client not available"),alert("Database connection not available. Please refresh the page.");return}console.log("🔍 Fetching fresh data directly from order_customizations table...");let{data:t,error:a}=await n.from("order_customizations").select("*").eq("id",s).single(),e=null,o=null;if(a){console.log("Not found in order_customizations, trying customization_forms...");const{data:m,error:v}=await n.from("customization_forms").select("*").eq("id",s).single();if(v){console.error("Error loading customization details:",v),alert("Error loading customization details: "+v.message);return}e=m,o=null}else console.log("✅ Found OMS customization in order_customizations table:",t),e={id:t.id,contact_email:t.email||"Not provided",email:t.email||"Not provided",product_type:"order-menu-system",product_name:"Order Menu System",project_name:t.project_name||"Not provided",restaurant_name:t.restaurant_name||"Not provided",owner_name:t.owner_name||"Not provided",restaurant_address:t.address_line_1||"Not provided",contact_person:t.contact_person||"Not provided",phone:t.phone_number||"Not provided",contact_phone:t.phone_number||"Not provided",additional_requirements:t.additional_requirements||"Not provided",primary_color:null,secondary_color:null,accent_color:null,text_color:null,product_price:t.total_amount||0,total_amount:t.total_amount||0,created_at:t.created_at,updated_at:t.updated_at||t.created_at,menu_categories:t.menu_categories||[],menu_items:t.menu_items||[],logo_url:t.restaurant_logo_url,logo_filename:t.logo_filename,logo_size:t.logo_size,city:t.city||"Not provided",state:t.state||"Not provided",pincode:t.pincode||"Not provided",country:t.country||"Not provided",house_flat_number:t.house_flat_number||"Not provided",menu_photos_urls:t.menu_photos_urls||[]},o=null;if(console.log("🔍 Database response:",{data:e,error:o}),console.log("🔍 Raw customization data from database:",e),o){console.error("❌ Error loading customization details:",o),alert("Error loading customization details: "+o.message);return}if(!e){console.error("❌ No customization found with ID:",s),alert("No customization found with the specified ID.");return}console.log("✅ Customization data loaded:",e);const d=document.getElementById("details-modal"),i=document.getElementById("modal-title"),r=document.getElementById("modal-content");if(console.log("🔍 Modal elements found:",{modal:!!d,modalTitle:!!i,modalContent:!!r}),!d||!i||!r){console.error("❌ Modal elements not found"),alert("Modal elements not found. Please refresh the page.");return}let c=e.menu_photos_urls||[];try{typeof c=="string"&&(c=JSON.parse(c))}catch{}const l=e.logo_url||e.restaurant_logo_url||"";let g=!1;(!Array.isArray(c)||c.length===0)&&l&&(c=[{url:l,filename:"restaurant-logo"}],g=!0);const y=Array.isArray(c)&&c.length?`<div class="grid grid-cols-3 sm:grid-cols-4 gap-3">${c.map(m=>`
          ...
          `).join("")}</div>`:'<p class="text-gray-500">No menu photos uploaded</p>',x=Array.isArray(c)&&c.length>0,$=(!!e.logo_url||!!e.restaurant_logo_url)&&!x&&!g,E=$?`<img src="${e.logo_url||e.restaurant_logo_url}" alt="Restaurant Logo" class="w-20 h-20 object-cover rounded border"/>`:"";i.textContent="Customization Details",r.innerHTML=`
        <div class="space-y-4">
          <!-- Product Information Section -->
          <div class="bg-gray-50 p-4 rounded-lg">
          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                ${E||'<span class="text-gray-500">Not uploaded</span>'}
              </div>
              `:""}
              <div>
